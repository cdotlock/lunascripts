...
description: >
  ...
  the MobAI game engine, whether authored manually or generated by a tool.
  Triggers on: LS scripts, episode scripts, visual novel scripts, game scripts,
  "@episode", "@choice", "@gate", or any mention of the MobAI script format.
---
...
**Every episode has exactly one `@gate { ... }` block.** No `@gate` = `MISSING_TERMINAL` validator error.
...
```
@bg school_hallway fade              // @ starts a new step group
...
Visual directives use the forms below.
...
@mauricio arms_crossed_angry dissolve // Switch pose
...
**Transitions:** optional; accepted values are `dissolve`, `fade`, `cut`, and `slow` for both character poses and backgrounds.
...
@bg school_front fade
@bg school_hallway cut
@bg dream_sequence slow
```
...
`@` starts a sequential step. `&` joins a single-line directive to the preceding `@` group. `&` cannot lead a sequence and cannot prefix dialogue or block directives such as `choice`, `phone`, `if`, `gate`, or `episode`.
...
Wait for one player click before advancing. `@pause` takes no arguments.
...
- `@text from <CHAR>: content` — `<CHAR>` is the sender's character ID.
- `@text to <CHAR>: content` — `<CHAR>` is the recipient's character ID.
- Character IDs are normalized to lowercase in JSON; the UI resolves display names from those IDs.
...
A `@choice` contains at least two `@option` blocks. Each option has an ID (A, B, C...) and a mode (`brave` = triggers D20 check, `safe` = no check).
...
- Must contain a `check { attr: ... dc: ... }` block. `attr` is non-empty and `dc` is a positive integer.
...
- `@butterfly` description (what happened as a result of this choice for downstream content generation)
...
- ❌ `@signal mark EASTON_ACKNOWLEDGED` as a character-moment marker with no follow-up — use `@butterfly` when the beat is only downstream-generation context
...
**Name every author signal (`mark` and `int`) and every achievement id in `SCREAMING_SNAKE_CASE` English.** Achievement IDs must match `^[A-Z][A-Z0-9_]*$`.
...
### `@butterfly` — downstream-generation memory, not routing

Butterfly records describe player-specific behavior for downstream content generation. They do not participate in gate routing. `@gate` evaluates deterministic state: `@signal mark`, `@signal int`, `@affection`, choice history, and engine values. There is no `influence` condition.
...
| Field | Type | Rule |
|-------|------|------|
| `id` | identifier | Must match `^[A-Z][A-Z0-9_]*$` |
...
Both sides of a comparison are operands. Either side can be any operand kind, so variable-to-variable, aggregate-to-literal, and literal-to-aggregate comparisons are supported.
...
`MAX` / `MIN` are **reserved words** and **must be uppercase**. Lowercase `max` / `min` may only appear as runtime-declared value names; author signal names must be `SCREAMING_SNAKE_CASE` and cannot be `MAX` or `MIN`.
...
Each episode has exactly one `@gate` block declaring where the player goes next. Use `@if` → `@else @if` → `@else` chains. First match wins. `@else` is the fallback.
...
1. **Missing `@gate`** — Every episode must have exactly one `@gate { ... }` block. No gate = `MISSING_TERMINAL` validator error.
2. **Multiple `@gate` blocks** — Exactly one per episode.
...
6. **Non-option content directly inside `@choice`** — A choice contains only `@option` blocks and must have at least two options.
...
9. **Nested `@choice` blocks** — `@choice` may contain only direct `@option` children; put narrative steps inside an option body.
10. **Using `&` on block structures** — `&choice`, `&phone`, `&if`, `&gate` are all errors. Block structures always use `@`.
...
23. **Using `check.success` outside a brave option body** — this is a validator error because the pseudo-identifier is context-local.
24. **Routing on butterfly / "influence"** — there is no `influence` condition. Butterflies feed downstream content generation, not gate routing.
25. **Single-arg `MAX(x)` or `MIN(x)`** — aggregate operators require `args >= 2`. A 1-arg aggregate is a parse error; write the arg directly instead.
26. **Lowercase `max(...)` / `min(...)` used as the aggregate operator** — `MAX` and `MIN` are reserved words and must be uppercase. Lowercase forms are not valid author signal names.
27. **Bundle of `@phone` block with non-`@text` content** — only `@text from/to` lines may appear inside `@phone { ... }`. No narration, no SFX, no `@if`, no state changes. Move those outside the phone block.
28. **Inventing character positions** — there is no syntax for `at left` / `at right` / `at center`. The frontend derives placement from MC identity. Use pose names, bubbles, and transitions for staging.
29. **Orphan branches and dead `@next` targets** — check these at content-review time; the compiler does not verify cross-file route targets.
...
- `references/LS-SPEC.md` — complete syntax and compiler contract. Read it for authoritative grammar, validation, and asset mapping behavior.
