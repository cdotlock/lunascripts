...
- **未入选及原因**（防止以后反复加）：
  - `nod` / `turn-away` / `close-eyes` 等摄像头 trick——需要运行时摄像头权限，权限墙太重，对一个强制叙事节拍代价不值；运行时拒绝 = 玩家卡死。
  - `hold-still` 稳住——双重失败：放在触摸 modality 里（按住屏幕别松）与 `hold` 同信号源、重叠；放在运动 modality 里用陀螺仪检测设备静止 = 检测玩家默认状态（玩手机本来就是静止持机），对玩家没有任何身体动作要求，不构成 trick。"屏息别动"的紧张感由 `hold`（按住别松）承担。
  - `trace` 描画——不要路径跟随，swipe 已覆盖方向手势。
  - `smile` 微笑、其他面部识别——又飘又需要摄像头权限。
  - `shout` / voice 喊、`blow` / `be-silent` 等麦克风模态——环境噪声下检测最不可靠，强制门承受不起；公共场合也尴尬。
  - drag / twist / scratch / cover / flip / raise / hum / lean-in / rotate / aim / steer 等候选——考虑过，未入选，保持 6 个。

**权限契约**：

- 触摸 trick：无权限。
- 运动 trick（`shake` / `swing` / `tilt`）：原生裸陀螺仪 / 加速度大概率**不弹运行时窗**（运行时弹窗是给计步 / 运动健康的，不是这类几何 / 状态读取）。具体由 Loopit 端在最新 iOS 上验证。
- 摄像头：MSS 不用，6 个 trick 类型里没有摄像头模态。
- 麦克风：MSS 不用，6 个 trick 类型里没有麦克风模态。
...
- **minigame 是 leaf step**——字段：`type`（`"minigame"`）、`name`（素材句柄）、`description`（喂给下游生成 agent 的 prose）、`game_url`（解析后的 OSS URL，未生成时为空字符串）。
...
| `@trick <type> "<prompt>"` | 强制 trick（leaf；引擎原生；type ∈ tap / hold / swipe / shake / swing / tilt） |
| `@choice { }` | 选择块 |
