...
引擎在运行时知道谁是 MC（从 gamestate 注入），编译产物**不携带 MC 身份信息**——MSS 对小说哪个角色是 MC 完全 agnostic，由前端业务层注入。
...
mss decompile ep01.json                                      # 反编译为 mss.md + assets_mapping.json
