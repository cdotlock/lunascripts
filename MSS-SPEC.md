...
MSS 把"打断剧情让玩家做点什么"拆成四个边界清晰的原语，按强制性 / 评级 / 奖励 / 叙事分支四个维度区分：

| 原语 | 强制性 | 评级 | 奖励 | 叙事分支 | 实现 |
|---|---|---|---|---|---|
| `@trick`(A) | 强制卡关 | 无 | 无 | 无 | 引擎原生（触摸 / 运动 / 摄像头） |
| `@minigame`(B) | 可选·可跳 | 无 | 有（引擎侧） | 无 | WebView，下游 agent 生成 |
| `@choice` + `brave`/`safe` | 强制 | D20 / 无 | — | 有（真分支） | 引擎原生 |
| `@cg`（见 §4.2） | — | — | — | body 对白 | 下游视频生成 |

> 历史背景：2026-05-19 前，`@minigame` 同时承担"小游戏"和"技能检定修正"两种职责，body 内带 `@if (rating.X)` 分支并可影响下一次 D20 检定。现在把这两条职责拆开——`@trick` 接管强制体感，`@minigame` 退化为"可选生成的小游戏"，D20 检定不再有小游戏修正项（见 §4.6.4）。
...
- 一行式，叶子指令，**无 body**。
- `<type>` 必须是下表 9 个锁定值之一；脚本不能新增、不能覆写。
...
| `hold-still` | 稳住 | 运动 | 无运行时弹窗 | 稳住手 / 屏息 / 僵住别被发现 |
| `nod` | 点头 | 摄像头 | **需摄像头权限** | 默认 / 答应 / 致意 |
| `turn-away` | 转头避视 | 摄像头 | **需摄像头权限** | 不敢看 / 回避 / 害羞 |
| `close-eyes` | 闭眼 | 摄像头 | **需摄像头权限** | 屏蔽画面 / 祈祷 / 入睡 |

**选型原则**（锁死依据）：

- trick 集是**调色盘**，覆盖情绪寄存器：动作（tap / shake / swing）、克制（hold）、紧绷躲藏（hold-still）、方向手势（swipe）、社交答复（nod / turn-away）、温柔（close-eyes）。
- **可靠性是及格线**，不是加分项——trick 是强制门，检测失败 = 玩家卡死。9 个全部是高可靠模态（触摸 / 运动 / 摄像头几何与状态），不含表情识别、不含麦克风。
- **未入选及原因**（防止以后反复加）：
  - `trace` 描画——不要路径跟随，swipe 已覆盖方向手势。
  - `tilt` 倾斜——叙事场景太窄。
  - `smile` 微笑——表情识别飘，强制门要可靠。
  - `shout` / voice 喊——公共场合尴尬。
  - `blow`、`be-silent`、整个麦克风模态——环境噪声下检测最不可靠，强制门承受不起；blow 还偏窄。
  - drag / twist / scratch / cover / flip / raise / hum / lean-in / tilt-head 等候选——考虑过，未入选，保持 9 个。

**权限契约**：

- 触摸 trick：无权限。
- 运动 trick（`shake` / `swing` / `hold-still`）：原生裸陀螺仪 / 加速度大概率**不弹运行时窗**（运行时弹窗是给计步 / 运动健康的，不是这类几何 / 状态读取）。具体由 Loopit 端在最新 iOS 上验证。
- 摄像头 trick（`nod` / `turn-away` / `close-eyes`）：需摄像头权限，**唯一的运行时权限墙**。检测**全程端上、一帧不离开手机**——这句话要进权限弹窗文案。
- 麦克风：**砍掉**，无麦克风 trick。
- 权限获取由 Loopit 负责，走原生 OS 权限（Info.plist），非 web 权限。

**与 `@pause for N` 的关系**：`@pause` 是隐形节奏（等 N 次点击），`@trick tap` 是有 prompt 与演出的实体动作——叙事意义不同，并存。

#### 4.6.2 `@minigame <name> "<description>"` — 独立小游戏

**定位**：嵌在剧情里的独立小游戏，玩完给奖励。可选——剧情把它递到面前，玩家可玩可跳。

**重大变化（2026-05-19）**：小游戏改为**生成**——由下游 vibe-coding agent 按描述自动生成，不再从预制 H5 库里选。CG 与 minigame 现在在描述方式上对称：都是 prose → 下游生成 agent（CG → 视频管线；minigame → vibe-coding agent）。
...
- `<name>`：素材句柄，同 `@cg show <name>`——生成完把 URL 填进素材表的 `assets.minigames.<name>`。
- `<description>`：**一整段连贯 prose，不分字段、不是 JSON**。同时干两件事：
  1. **衔接剧情**——什么场合、为何有这局、赌注 / 氛围、谁在场。
  2. **简单定义玩法**——玩家做什么、核心机制、怎样算赢。"简单"是关键：给方向，别写成设计文档。
- **无 body**——叶子指令。
- **无属性**（无 `<ATTR>`）——脱钩 D20 检定（见 §4.6.4）。
- **无评级分支**（无 `@if (rating.X) { }`）——`rating.<grade>` 已从合法 condition 类型中删除。
- **奖励全在引擎侧**——脚本不写金额、不写消耗。引擎根据 H5 回传的分数缩放奖励（反作弊在此），脚本保持精简。
- **跳过 = 整条 `@minigame` 当 no-op**，从其后继续，无奖励。

**describe 的质量靠 SKILL 层**：写 describe 的 Dramatizer 必须知道 vibe-coding agent 能生成多复杂的游戏——这套"可生成玩法、复杂度上限"的指引放进喂给 Dramatizer 的 prompt（详见 `skills/mss-scriptwriting/SKILL.md` 的 Mini-games 节），**不进 MSS 语法**。
...
> **2026-05-19 变更**：原公式有第三项"小游戏修正"，与 `@minigame` 的 ATTR 绑定。trick / minigame 解耦后，`@minigame` 不再绑定属性、不再影响 D20，第三项已彻底删除。XP / SAN / reroll / coins / gems 等数值经济**全部不动**，本次只解耦检定层。砍掉整个 D20/DC 检定层是更大的决定，留待将来单独评估。
...
**条件类型（6 种，body `@if` 和 gate `@if` 均可使用）。所有类型都解析为结构化 AST，后端直接消费：**
...
> **2026-05-19 变更**：第 7 种 condition `rating.<grade>` 已删除——`@minigame` 不再带 body，没有评级分支，自然不需要 `rating` condition。解析器遇到 `rating.X` 会给迁移提示并报 parse error。
...
`@trick` 与 `@minigame` 是叶子指令（无 body），原则上**允许 `&` 并发前缀**——可以与 `@bg set` 等同组——但叙事上几乎总是想让它独占一个步骤，建议保持 `@` 顺序执行，避免与其他动作混入同一组。
...
| `@minigame qte_challenge "..."` | `assets.minigames.qte_challenge` | `{base_url}/minigames/qte_challenge/index.html`（由下游 vibe-coding agent 生成后填入） |
| `@trick tap "..."` | — | 无素材（引擎原生） |
...
mss decompile ep01.json                                      # 反编译为 ep01_decompiled/mss.md + assests_mapping.json
...
5. **反编译**：从已编译 JSON 恢复 MSS 源文件，并从 URL 字段抽取素材映射
...
- 嵌套只出现在有分支的地方（choice.options、cg_show.steps、if.then/else、gate if/else、phone_show.messages）。`minigame` 与 `trick` 都是 leaf step，没有 child container。
...
- **minigame 是 leaf step**——字段：`type`（`"minigame"`）、`name`（素材句柄）、`description`（喂给下游生成 agent 的 prose）、`game_url`（解析后的 OSS URL，未生成时为空字符串）。无 `attr`，无 `game_id`，无 `steps`/`body`，无评级分支。
- **trick 是 leaf step**——字段：`type`（`"trick"`）、`trick_type`（9 个锁定值之一）、`prompt`（玩家面看到的祈使）。无 URL，无 body——引擎原生。引擎遇到 trick 步骤必须**阻塞**直到玩家完成。
- CG 块输出 `duration`（`low`/`medium`/`high`）和 `content`（英文叙事串）字段；body 步骤在 `steps` 字段下
- Achievement 是 inline step，step 本身携带完整元数据：`{"type":"achievement","achievement_id":"...","name":"...","rarity":"...","description":"..."}`。语义 id 用 `achievement_id` 字段——避免与编译器在所有 step 上注入的后端 cursor `id` 字段冲突（cursor `id` 是后端契约、详见 `docs/JSON-OUTPUT.md` §4.0，作者侧无需关心）。
...
| `@music fadeout` | 淡出停止 |
| `@sfx play <name>` | 播放音效 |
| `@minigame <name> "<description>"` | 可选小游戏（leaf；下游 vibe-coding agent 按 description 生成；可玩可跳；无 ATTR、无 body、无 rating） |
| `@trick <type> "<prompt>"` | 强制 trick（leaf；引擎原生；type ∈ tap / hold / swipe / shake / swing / hold-still / nod / turn-away / close-eyes） |
| `@choice { }` | 选择块 |
