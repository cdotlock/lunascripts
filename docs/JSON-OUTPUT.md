...
|------|------|
| `choice` | 选择菜单，等待玩家选择 |
| `minigame` | 可选小游戏，等待游戏结果（玩家可跳，跳则当 no-op） |
| `trick` | 强制 trick（引擎原生检测），等待玩家完成；不能跳 |
| `phone_show` | 手机界面 |
| `phone_hide` | 关闭手机 |
| `cg_show` | CG 展示（内含子步骤） |
| `if` | 条件分支（引擎判定后执行对应分支） |
...
子容器（`choice.options[i].steps`、`cg_show.steps`、`if.then`/`if.else`、`phone_show.messages`）的子步骤在父步骤被编号之后继续递增。`minigame` 与 `trick` 都是 leaf step——它们各自消耗一个 seq，但**不开 child counter**，下一个兄弟步骤直接在父容器的计数器里继续。

**并发组**：MSS 的 `&` 跟随节点会和 `@` 领导节点合并成一个 JSON 数组（见 §2.2），组内成员在同一个计数器里**继续递增**。
...
steps[4] = 这个 0005_ch choice 的 options[1].steps（Safe 选项 B）:
  0009_dlg, 0010_dlg, 0011_nar
  ^^^^^^^^
  从 option A 之后继续递增（不重启）
```

#### 唯一性

由于 Episode 全局作用域，同一份 JSON 中**不会出现重复 id**——`sort(allStepIDs) == DFS 声明顺序`。后端 player cursor 既可以用 id 直接定位（全局唯一），也可以用完整路径定位。路径形式示例：
...
```json
{
  ...
  "description": "Mr. Chen pairs Malia and Mauricio for the opening reading exercise — they trade lines from Jane Eyre while the rest of the class watches to see who flinches first. The player taps to match Malia's pacing to a rhythm meter..."
}
```

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `name` | string | 是 | 素材句柄（同 `@cg show <name>`），用于查 `assets.minigames.<name>` |
| `game_url` | string | 否 | 已解析的小游戏 URL。生成器尚未完成时可缺省或为空字符串 |
| `description` | string | **是** | 连贯英文 prose：既描述场景（为何这里有这局），也定义简单玩法（玩家做什么、怎样算赢）。这是喂给下游 vibe-coding agent 的输入；运行时播放器可忽略 |

> **从旧格式迁移**：旧 `minigame` step 带 `game_id` / `attr` / `steps` 三个字段，分别表达"小游戏 ID""绑定属性""按评级分支的子剧本"。新模型把 `game_id` 改名为 `name`、删除 `attr`（minigame 不再影响 D20 检定）、删除 `steps`（无评级分支）。解码工具 (`mss decompile`) 兼容历史 payload 的 `game_id` 字段。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `trick_type` | string | 是 | 9 个锁定值之一：`tap` / `hold` / `swipe` / `shake` / `swing` / `hold-still` / `nod` / `turn-away` / `close-eyes`。其它值是 parser/validator error |
| `prompt` | string | 是 | 一行祈使，给玩家看（并兼作叙事粘合）。不可为空串 |

阈值（点几下、按多久等）由引擎内置，**脚本不传**。摄像头类型（`nod`/`turn-away`/`close-eyes`）需要摄像头权限——一帧不离开手机，由 Loopit 负责权限请求与文案。
...
> **注意 `id` vs `achievement_id`：** 与所有 step 一样，`achievement` 也带通用的 `id` 字段（cursor 稳定步骤 id，格式 `<seq>_ach`，见 §4.0）；`achievement_id` 才是 MSS 源里 `@achievement <id>` 写下的语义标识（如 `"RARE_COURAGE"`）。两者不可互换：执行器/UI 用 `achievement_id` 记录解锁状态，cursor 用 `id` 定位。`minigame` step 同理用 `name` 携带素材句柄，通用的 `id` 字段是 cursor 锚点。
...
> **2026-05-19 变更**：`rating` 条件已从语言中删除。`@minigame` 不再带 body，没有评级分支需求。parser 遇到 `rating.X` 会给迁移提示。
...
**gate 中不会出现 `check` 条件**——它只在 brave option 体内的 body `@if` 里生成。编译器不强制作用域，只是剧作者写在 gate 里对路由无意义（运行时恒 false）。
