...
> **2026-05-19 变更**：D20 公式去掉了"小游戏修正"项。`@minigame` 不再与 D20 检定耦合——见 `minigame` step 一节。
> 同期 `rating.<grade>` 条件 与 minigame body 一起删除——`minigame` 现在是 leaf step，没有评级分支需求。
...
2. 引擎依据 `check.attr` 和 `check.dc` 掷 D20（仅加属性修正——2026-05-19 起取消小游戏修正项）
...
- **`name`**：素材句柄，与 `assets.minigames.<name>` 对应。decompiler 也兼容历史 payload 中的 `game_id` 字段。
...
- 旧 `minigame` step 有 `attr` 字段，参与 D20 检定的"小游戏修正项"。**已删除**。
- 旧 `minigame` step 有 `steps` 子容器，里面是按 `@if (rating.X)` 分的子剧本。**已删除**——`rating` 条件 与 `minigame.steps` 一起取消。
- 旧 `game_id` 字段更名为 `name`。
- 奖励完全在引擎侧。脚本不写金额、不写消耗。
...
| `nod` | 摄像头 | 摄像头 | 面部俯仰角检测点头 |
| `turn-away` | 摄像头 | 摄像头 | 面部偏航角偏离正前 / 离开画框 |
| `close-eyes` | 摄像头 | 摄像头 | 眼睑闭合状态 |

**阈值由引擎拥有**——脚本不传，未来也不允许传。摄像头类型（`nod` / `turn-away` / `close-eyes`）需要摄像头权限，**首次使用前**由 Loopit 申请；检测帧**全程端上、一帧不离开手机**，权限弹窗文案要明示这一点。
...
> **2026-05-19 变更**：`rating` 条件已删除——`@minigame` 现在是 leaf step，无 body / 无评级分支，不需要 rating context。如果旧 JSON payload 还携带 `{"type":"rating", ...}` 条件，把它当作未知 condition type 处理（返回 false 并记日志）。
...
`ctx` 是引擎维护的当前集临时状态，包含 `ctx.check`（当前 brave option 的检定结果，离开 option 体时清空）。`rating` 条件已删除——若旧 payload 出现，按未知 condition 处理（return false + log）。
...
function handle_choice(step):
    ...
    if opt.mode == "brave":
        # D20 公式：roll + 属性修正。2026-05-19 起没有小游戏修正项。
...
function handle_minigame(step):
    ...
    # 无 ctx.rating，无 check_modifier — 这两个机制 2026-05-19 已删除
```
...
function handle_trick(step):
    detector = engine.start_trick_detector(step.trick_type)  # 9 选 1
